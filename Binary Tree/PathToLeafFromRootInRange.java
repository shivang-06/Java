...
public class Main {
    ...
    public static void pathToLeafFromRoot(Node node, String psf, int ssf, int lo, int hi) {
        ...
        psf = psf + node.data +" "; //path so far
        ssf += node.data; // sum so far

        if (node.left == null && node.right == null && (ssf >= lo && ssf <= hi)) {
            System.out.println(psf);
            return;
        }
        
        pathToLeafFromRoot(node.left, psf, ssf, lo, hi);
        pathToLeafFromRoot(node.right, psf, ssf, lo, hi);
