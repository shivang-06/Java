...
public class Main {
    ...
    public static class diaPair {
        int diameter;
        int height; //distance between node and farthest node
    }
    public static diaPair diameter2(Node node) {
        ...
        diaPair lPair = diameter2(node.left);//f2
        diaPair rPair = diameter2(node.right); //f3
        
        int localDia = lPair.height + rPair.height + 2;//f1
        int localHt = Math.max(lPair.height,rPair.height) + 1;
        diaPair res = new diaPair();
        res.diameter = Math.max(Math.max(lPair.diameter,rPair.diameter),localDia);
        res.height = localHt;
