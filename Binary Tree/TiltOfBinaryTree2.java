...
public class Main {
    ...
    public static tPair tilt1(Node node) {
        ...
        tPair lPair = tilt1(node.left);
        tPair rPair = tilt1(node.right);
        int localTilt = Math.abs(lPair.sum-rPair.sum); //tilt of node
        tPair res = new tPair();
        res.sum = lPair.sum+rPair.sum + node.data;
        res.tilt = localTilt +rPair.tilt + lPair.tilt;
        return res;
    }
    public static void main(String[] args) throws Exception {
        ...
        tPair ans = tilt1(root);
        System.out.println(ans.tilt);
    }
